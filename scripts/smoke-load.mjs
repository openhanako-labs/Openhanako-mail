...
check("native profile 与 manifest 一致",
  rtHost.includes('startWith("native")') && caps.includes("app/runtime.native"));
// native 建不起沙箱时（HANA_HOME 上有重解析点）会降级到 local-machine，
// 两侧都得对得上：源码里确实会降级，manifest 里确实授权了。
check("local-machine 降级路径与 manifest 一致",
  rtHost.includes('startWith("local-machine")') && caps.includes("app/runtime.local-machine"));
