...
function hintFor(err) {
  ...
  if (/native-identity|reparse root/i.test(s)) {
    return "宿主创建受管沙箱身份失败：安装路径或 HANA_HOME 上存在符号链接/重解析点。"
      + "本应用已把 entry/cwd/args 解成真实路径仍无法规避，需宿主侧修复，"
  ...
  _starting = (async () => {
    try {
      ...
      const rec = await _ctx.runtime.start({
        runtime: "node",
        // 绝对真实路径：避免 native-identity 在含符号链接的路径上做 realpath 时
        // 报 "reparse root is not supported"（解析失败时退回原相对路径）。
        entry: ENTRY_PATH,
